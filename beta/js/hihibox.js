(function(window,document,undefined) {
	...
	var HihiBox = function(settings) {
		...
		var _this = this,
			...
			info = _settings.info,
			defaultGenre = _settings.defaultGenre,
			env = {
				hasjQuery: false,
				platform: '',
			...
			hitbox: function() {
				...
				_platform.toggleDarkMode = function(act) {
					if (act == 'dark') {
						...
						$(selector.darkModeBtn).text('Light Mode');
					} else if (act == 'light') {
						if ($(selector.darkModeAcceptor).hasClass(cssClass.darkMode)) {
							$(selector.darkModeAcceptor).removeClass(cssClass.darkMode);
						}
						$(selector.darkModeBtn).text('Dark Mode');
					} else {
						if ($(selector.darkModeAcceptor).hasClass(cssClass.darkMode)) {
		...
		var bindIconList = function() {
			...
			$.each(listIcon,function(idx,obj) {
				palIconset.append(
					$('<div class="'+cssClass.icon+'" hhb-code="'+obj.code[0]+'" hhb-genre="'+obj.genre.join(' ')+'"></div>')
			...
			selectGenre(defaultGenre);	// select default genre
		};
		...
		var toggleDarkMode = function(act) {
			platformObj.toggleDarkMode(act);
		};
			...
			if (msg) {
				if (msg.length >= filter.minLength) {
					...
					if (recodehead.length >= filter.minLength) {
						...
						$icons.not(selector).removeClass([cssClass.filterMatch,cssClass.filterSelected].join(' ')).addClass(cssClass.filterNotMatch);
						var filtered = $icons.not(selector.iconMissing).filter(fselector)
							.removeClass([cssClass.filterNotMatch,cssClass.filterSelected].join(' ')).addClass(cssClass.filterMatch)
						...
						showIconMsg();
						debugMsg('Filtered [C:',codehead,',F:',filtered.length,']');
					}
				}
				if (!isFiltering) {
					toggleHolder('hide');
