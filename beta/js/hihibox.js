(function(window,document,undefined) {
	var ojQuery = jQuery;	/* backup original jQuery */
	/* HihiBox jQuery */
	...
	var HihiBox = function(settings) {
		...
		_this._platformObj = {
			...
			hitbox: function() {
				...
				_platform.getHolderContainer = function() {	return $(_platform.selector.holderContainer);	};
				_platform.getButtonContainer = function() {	return $(_platform.selector.buttonContainer);	};
				_platform.genHolder = function(idObj,infoObj) {
					return _this._platformObj.default.genHolder(idObj,infoObj);
				};
				_platform.genToggleButton = function(id) {
					return $('<div id="'+id+'" class="icon-cog hoverG2" title="HihiBox"></div>');
				};
				_platform.displayToggleButton = function() {};
				_platform.getShowChatBtn = function() {	return $(_platform.selector.showChatBtn);	};
				_platform.toggleDarkMode = function(act) {
					if (act == 'dark') {
						if (!$(_platform.selector.darkModeAcceptor).hasClass(_platform.class.darkMode)) {
							$(_platform.selector.darkModeAcceptor).addClass(_platform.class.darkMode);
						}
						$(_platform.selector.darkModeBtn).text('Light Mode');
					} else if (act == 'light') {
						if ($(_platform.selector.darkModeAcceptor).hasClass(_platform.class.darkMode)) {
							$(_platform.selector.darkModeAcceptor).removeClass(_platform.class.darkMode);
						}
						$(_platform.selector.darkModeBtn).text('Dark Mode');
					} else {
						if ($(_platform.selector.darkModeAcceptor).hasClass(_platform.class.darkMode)) {
							_platform.toggleDarkMode('light');
						} else {
							_platform.toggleDarkMode('dark');
						}
					}
				};
				...
				_platform.injectIcon = function(iconlist,_hhb) {
				...
				_platform.getNewMsg = function() {	return $(_platform.selector.newMsg);	};
				_platform.bindResizer = function(_hhb) {
					...
					if (msgs.length > 0) {
						...
						msgs.each(function() {
							bicount = $(this)
								.addClass(_hhb.class.checkedMsg)
								.children(_hhb.selector.msgIconResized)
									.click(function() {
										$(this).toggleClass(_hhb.class.orgSize);
									}).length;
						});
					}
					return {
			...
			twitch: function() {
				...
				_platform.getPlatformIcon = function() {
					...
					for (var i=0;i<listCss.length;i++) {
						...
						var icon = cssitem.match(reBGimg);	icon = (icon) ? icon[1] : icon;
						var w = cssitem.match(reWidth);		w = (w) ? w[1] : w;
						var h = cssitem.match(reHeight);	h = (h) ? h[1] : h;
						if (cls && icon && w && h) listCssLookup[cls] = { cls: cls, icon: icon, width: w, height: h };
					}
				...
				_platform.injectIcon = function(iconlist,_hhb) {
					...
					while (icon = iconlist.pop()) {
						...
						style += ['.'+classname+'.'+_hhb.class.msgIcon+'{', 
								'opacity: initial;',
								'background-size:'+w+' '+h+';',
								'width:'+w+';',
								'height:'+h+';',
						...
						style += ['.'+classname+'{',
								...
								'}'].join(' ');
						style += ['.'+classname+','+
								'.'+classname+'.'+_hhb.class.msgIcon+'.'+_hhb.class.orgSize+'{',
				...
				_platform.bindResizer = function(_hhb) {
					...
					if (msgs.length > 0) {
						...
						msgs.each(function() {
							var emoticons = $(this)
								.addClass(_hhb.class.checkedMsg)
								.children(_platform.selector.emoticon)
									.each(function() {
										if ($(this).height() > limitHeight) $(this).addClass(_hhb.class.resized);
										$(this).addClass(_hhb.class.msgIcon);
									});
							resized = $(this).children(_hhb.selector.msgIconResized)
								.click(function() {
									$(this).toggleClass(_hhb.class.orgSize);
								});
						});
		...
		_this.analyzeIcon = function(iconlist) {
			...
			var codeIsDuplicated = function(code) {
				...
				for (var i=0;i<ncode.length;i++) if (_this.listLookup[ncode[i]]) return true;
				return false;
			};
			$.each(iconlist,function(idx,obj) {
				...
				for (var i=0;i<code.length;i++) _this.listLookup[code[i]] = obj;
		...
		_this.bindIconList = function() {
			var _this = this;
			if (!(_this.env.builtinIconLoaded || _this.env.platformIconLoaded)) return;
			...
			$.each(_this.listIcon,function(idx,obj) {
				palIconset.append(
					$('<div class="'+_this.class.icon+'" hhb-code="'+obj.code[0]+'" hhb-genre="'+obj.genre.join(' ')+'"></div>')
						.data('hhb-code',obj.code.join(' '))
						.click(function() {	_this.insertIcon($(this)); })
						.append($(obj.img)
							.error(function() {	$(this).parent().addClass(_this.class.iconMissing); _this.showIconMsg(); })
						)
				);
			});
		...
		_this.showIconMsg = function() {
			...
			if (si > 0) 
				if (_this.isFiltering) $iconMsgBox.text(['Filtering...( ',si,'/',i,' )'].join('')).clearQueue().show().fadeTo(fidur,fiop);
			...
			if (msg) {
				if (msg.length >= _this.filter.minLength) {
					...
					if (recodehead.length >= _this.filter.minLength) {
						...
						var filter = $icons.not(_this.selector.iconMissing).filter(selector)
		...
		_this.bindResizer = function() {
			var _this = this;
			var result = _this.platformObj.bindResizer(_this);
			if (result.msg > 0) 
				_this.debugMsg('Messages Parsed [M:',result.msg,',R:',result.resized,']');
			setTimeout(function() { _this.bindResizer(); },_this.delay.bindResizer);
		};
		...
		var timer = setInterval(function() {
			if (window.iconListLoaded) {
				var hhb = 	new HihiBox({
									...
									supportedPlatform: ['hitbox','twitch']
								});
